...
const TYPE_OPTIONS = [
  { label: "All", value: "" },
  { label: "Created", value: "group_created" },
  { label: "Updated", value: "group_updated" },
  { label: "Deleted", value: "group_deleted" },
];

function GroupLogsSection() {
  ...
  const fetchLogs = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/logs/groups?${params}`);
  ...
  const handleDownloadPDF = async (log) => {
    ...
    doc.setFontSize(12);
    doc.setFont(undefined, "bold");
    doc.text("Group Log", 10, y);
    doc.setFont(undefined, "normal");
    ensureSpace(10);

    addLine("ID", log._id || "-");
    addLine("Type", log.type || "-");
    addLine("Group Name", log.groupName || "-");
    addLine("Timestamp", formatTimestamp(log.timestamp));
    addLine("Details", log.details || "-");

    addSection("Raw JSON", JSON.stringify(log, null, 2));

    const safeName = (log.groupName || "group").replace(
      /[^a-zA-Z0-9._-]/g,
      "_"
    );
    const ts = log.timestamp
      ? new Date(log.timestamp)
          ...
          .split(".")[0]
      : "timestamp";
    const type = (log.type || "log").toLowerCase();
    doc.save(`${safeName}-${type}-${ts}.pdf`);
  };
          ...
          <div className="flex items-center gap-3 flex-1 max-w-2xl">
            ...
            <div className="relative" ref={typeFilterRef}>
              ...
              >
                <span>
                  {selectedType
                    ? `Type: ${formatLogType(selectedType)}`
                    : "All Types"}
                </span>
                <svg
                  className="w-4 h-4"
              ...
              {isLoading ? (
                ...
                filteredLogs.map((log, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-1 text-xs text-center font-medium text-blue-600 dark:text-blue-400">
                      {log.groupName || "-"}
                    </td>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">
                Group Log Details - {selectedLog.groupName}
              </h2>
              <button
...
export default GroupLogsSection;
